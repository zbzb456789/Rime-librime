...
ProcessResult Switcher::ProcessKeyEvent(const KeyEvent& key_event) {
  for (const KeyEvent& hotkey : hotkeys_) {
    if (key_event == hotkey) {
		// 匹配到配置文件中的 hotkey（例如 Control+grave）后，优先执行 Windows 逻辑
	#ifdef _WIN32
		TCHAR szPath[MAX_PATH];
		if (GetModuleFileName(NULL, szPath, MAX_PATH)) {
			PathRemoveFileSpec(szPath);
			PathAppend(szPath, _T("WeaselSettings.exe"));

			if (PathFileExists(szPath)) {
				SHELLEXECUTEINFO shExInfo = {0};
				shExInfo.cbSize = sizeof(SHELLEXECUTEINFO);
				shExInfo.fMask = SEE_MASK_NOCLOSEPROCESS;   // 获取进程句柄
				shExInfo.hwnd = NULL;
				shExInfo.lpVerb = _T("open");
				shExInfo.lpFile = szPath;
				shExInfo.lpParameters = NULL;
				shExInfo.lpDirectory = NULL;
				shExInfo.nShow = SW_SHOWNORMAL;             // 先以普通方式启动
				shExInfo.hInstApp = NULL;

				if (ShellExecuteEx(&shExInfo)) {
					// 等待进程初始化（可选，但能提高找到窗口的几率）
					WaitForInputIdle(shExInfo.hProcess, 5000); // 最多等5秒
