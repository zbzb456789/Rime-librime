...
class path : public std::filesystem::path {
 ...
 public:
  ...
  // Provide u8string and generic_u8string that always return std::string.
  // In some standard library implementations these return
  // std::basic_string<char8_t>, which is incompatible with existing code.
  std::string u8string() const {
    auto us = fs_path::u8string();
    std::string out;
    out.reserve(us.size());
    for (auto ch : us) {
  ...
  std::string generic_u8string() const {
    auto us = fs_path::generic_u8string();
    std::string out;
    out.reserve(us.size());
    for (auto ch : us) {
      out.push_back(static_cast<char>(ch));
  ...
  // Return rime::path for operations that would otherwise return
  // std::filesystem::path so chained calls use rime::path methods.
  path filename() const { return path(fs_path::filename()); }
  path extension() const { return path(fs_path::extension()); }
  path parent_path() const { return path(fs_path::parent_path()); }
  ...
  // Note: do not add overloads taking std::filesystem::path and
  // std::string/char* here because std::filesystem may already provide
  // such overloads with a different return type; declaring them would
  // conflict with existing overloads at namespace scope.
#ifdef RIME_ENABLE_LOGGING
