...
public:
  ...
  std::string FindConfigFile(std::string fileName) {
    ...
    // 1. 修改这一处
    ifs.open(std::filesystem::u8path(fileName)); 
    if (ifs.is_open()) { return fileName; }

    if (PACKAGE_DATA_DIRECTORY != "") {
        std::string prefixedFileName = PACKAGE_DATA_DIRECTORY + fileName;
        // 2. 修改这一处
        ifs.open(std::filesystem::u8path(prefixedFileName));
        if (ifs.is_open()) { return prefixedFileName; }
    ...
    for (const std::string& dirPath : paths) {
        std::string path = dirPath + '/' + fileName;
        // 4. 修改这一处
        ifs.open(std::filesystem::u8path(path));
        if (ifs.is_open()) { return path; }
    }
    throw FileNotFound(fileName);
	}
};
...
ConverterPtr Config::NewFromFile(const std::string& fileName,
  ...
  std::ifstream ifs(std::filesystem::u8path(prefixedFileName));
